...
{
    public class FloorPlanHandler : AuthorizationHandler<OperationAuthorizationRequirement, FloorPlan>
    {
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, OperationAuthorizationRequirement requirement, FloorPlan resource)
        {
            ...
            if (context.User.IsInRole(Constants.Role.BUILDING_MANAGER) && !resource.Building.AdminId.Equals(buildingManagerId))
            {
                context.Fail();
                return Task.CompletedTask;
            }
